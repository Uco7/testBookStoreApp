...
//     if (existing)
//       return res.status(409).json({ message: "User already exists with the email or userName" });
    
//     console.log("exit user:",existing)
//     // 5. Hash password
...
//     // Hash the code before saving to DB for security
//     user.resetPasswordOTP = crypto.createHash("sha256").update(otp).digest("hex");
//     user.resetPasswordExpires = Date.now() + 10 * 60 * 1000; // 10 minute expiry
//     await user.save();
...
import jwt from "jsonwebtoken";
import User from "../models/user.js";
import OtpUser from "../models/otpUser.js"; // You need to create this model
...
export const requestRegistration = async (req, res) => {
  try {
    ...
    // 4. Save to temporary collection (upsert based on email)
    await OtpUser.findOneAndUpdate(
      { email: cleanEmail },
      {
        ...
        otp,
        createdAt: Date.now(), // For TTL expiry
      },
