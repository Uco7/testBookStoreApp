...
// setupNotificationChannels(). Import the shared constant instead of
// retyping the string, so a future channel-ID bump can't be applied
// in one file and missed in another.
import { PREMIUM_ALARM_CHANNEL_ID } from "../../app/_layout";
...
export const scheduleOfflineTimetable = async (timetable) => {
  try {
    ...
    // FIX: use timetable.mode === "alarm" (same field the server push
    // path checks) instead of timetable.planType === "premium". A
    // "premium" plan with mode "notification" should NOT get the
    // alarm sound/channel, and vice versa — mode is the source of
    // truth for which sound/channel to use, planType is not.
    const isAlarm = timetable.mode === "alarm";

    // FIX: this loop previously scheduled exactly ONE trigger per
    // study day, completely ignoring noticeCount. The server-side
    // scheduleTimetableJobs() correctly fans each day out into
    // `noticeCount` triggers spaced 5 minutes apart (see jobIds count
    // on the timetable doc); this local scheduler needs the same
    // fan-out or noticeCount > 1 silently does nothing extra here.
    ...
    for (const day of days) {
      ...
      for (let i = 0; i < noticeCount; i++) {
        // Roll minute overflow into the hour correctly (e.g. 00:58 + 5min
        // -> 01:03), and wrap the hour back into 0-23 if it crosses midnight.
        const totalMinutes = hour * 60 + minute + i * SPACING_MINUTES;
        const noticeHour = Math.floor(totalMinutes / 60) % 24;
        const noticeMinute = totalMinutes % 60;
        ...
        const notificationId = await Notifications.scheduleNotificationAsync({
          content: {
            title: isAlarm
              ? `⏰ Study time: ${timetable.bookTitle}${noticeLabel}`
              : `📚 Reminder: ${timetable.bookTitle}${noticeLabel}`,

            body: timetable.notificationMessage || "Study time",


            // FIX: include the .wav extension so this matches exactly
            // what the channel was created with and what the remote
            // push path sends — local notification sound resolution on
            // Android/iOS expects the same filename string used at
            // channel-registration / native-bundle time.
          ...
          trigger: {
            ...
            hour: noticeHour,
            minute: noticeMinute,
            // FIX: Android resolves a notification's sound/importance
            // from its channel, not from `content.sound` alone. Without
            // explicitly setting channelId here, this trigger falls back
            // to the "default" channel on Android and the alarm sound
            // never plays regardless of the `sound` field above.
            ...(isAlarm ? { channelId: PREMIUM_ALARM_CHANNEL_ID } : {}),
          },
        });
