...
// // ─────────────────────────────────────────────
// // ⏱️ Guard so any single boot task can never block the splash forever
// // ─────────────────────────────────────────────
...
//     new Promise((resolve) =>
//       setTimeout(() => {
//         console.log(`⏱️ ${label} timed out after ${ms}ms — continuing anyway`);
//         resolve();
//       }, ms)
//     ),
//   ]);
...
//   // ─────────────────────────────────────────────
//   // 🔑 THE ACTUAL FIX: mobileAds().initialize() is fire-and-forget here,
//   // NOT awaited by anything that blocks the splash. In production, on a
//   // real (non-test) device, this call can wait on a native GDPR/consent
//   // dialog that the opaque splash overlay (zIndex 999) hides from view —
//   // making the app look permanently frozen. Removing it from the boot
//   // gate means the splash can never wait on it again.
//   // ─────────────────────────────────────────────
//   useEffect(() => {
//     mobileAds()
//       .initialize()
...
//   useEffect(() => {
//     // ─────────────────────────────────────────────
//     // BOOT SEQUENCE — mobileAds().initialize() is intentionally NOT
//     // called here anymore. Only setupNotificationChannels() (guarded
//     // with a 4s timeout) gates the splash now, so it always clears in
//     // a few seconds regardless of what AdMob does.
//     // ─────────────────────────────────────────────
//     (async () => {
//       try {
//         setProgress(0.3);
//         await withTimeout(setupNotificationChannels(), 4000, "Notification channel setup");
//         setProgress(0.7);
...
export default function RootLayout() {
  ...
  // ─────────────────────────────────────────────
  // 🔑 THE ACTUAL FIX FOR THE STUCK SPLASH:
  // Previously, hiding the native splash was wired to the container
  // View's onLayout event — but onLayout only fires on real layout
  // events (initial mount, size changes), NOT just because `appReady`
  // changed. Since the very first layout event happens almost
  // immediately (while appReady is still false), and the container
  // never resizes again afterward, SplashScreen.hideAsync() was NEVER
  // actually being called — regardless of how long the boot sequence
  // took. Confirmed via logcat: the native "Splash Screen" buffer
  // surface was created on launch and never disconnected, even many
  // seconds after JS had fully booted and ads had loaded.
  //
  // Fix: hide the native splash directly from an effect keyed on
  // appReady, with no dependency on any layout event at all.
  // ─────────────────────────────────────────────
  useEffect(() => {
    if (!appReady) return;
