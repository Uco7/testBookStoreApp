...
// ─── NEW: date helpers for search + display ───────────────────────────────────
const MONTHS = [
...
export default function TimetableScreen() {
  ...
  // ── NEW: tracks live 0–100 progress per item so the UI can show a real bar ──
  ...
  // ── UPDATED: search now checks title, file name, file type, and date tokens ──
  const filtered = (timetables || []).filter((t) => {
  ...
  const renderItem = ({ item }) => {
    ...
    const createdLabel = formatDisplayDate(item.createdAt); // ── NEW ──
    ...
    return (
      ...
      >
        ...
        {/* ── NEW: file name + created date row ── */}
